 ...
 * @reset_port: pcie port reset callback
 * @phy_resets: phy reset lines SoC data.
 */
struct mtk_gen3_pcie_pdata {
	int (*power_up)(struct mtk_gen3_pcie *pcie);
	void (*reset_port)(struct mtk_gen3_pcie *pcie);
	struct {
...
static void mtk_pcie_reset_port(struct mtk_gen3_pcie *pcie)
{
	u32 val;
...
static int mtk_pcie_startup_port(struct mtk_gen3_pcie *pcie)
{
	...
	/* Reset the PCIe port is requested by the hw */
	if (pcie->soc->reset_port)
		pcie->soc->reset_port(pcie);
...
static const struct mtk_gen3_pcie_pdata mtk_pcie_soc_mt8192 = {
	...
	.reset_port = mtk_pcie_reset_port,
	.phy_resets = {
		.id[0] = "phy",
		.num_resets = 1,
	},

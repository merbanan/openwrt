 ...
 *
 * Data model and datapath mirror the mainline Airoha ethernet driver
 * (drivers/net/ethernet/airoha/airoha_eth.c by Lorenzo Bianconi), adapted
 * to the EcoNet EN751221 QDMA (single IRQ bank / TX done queue per engine,
 * two CPU rings, register offsets per the EN751221 programming guide).
 */
...
/* EN751221 exposes two CPU rings per QDMA engine */
...
/* One TX "done"/IRQ queue per QDMA engine on EN751221 */
#define ECONET_NUM_TX_IRQ		1

#define ECONET_RX_HEADROOM		(NET_SKB_PAD + NET_IP_ALIGN)
#define ECONET_RX_LEN(_n)		((_n) - ECONET_RX_HEADROOM)

#define HW_DSCP_NUM			2048
#define IRQ_QUEUE_LEN(_n)		1024
...
enum econet_dma_map_type {
	ECONET_DMA_UNMAPPED,
	ECONET_DMA_MAP_SINGLE,
	ECONET_DMA_MAP_PAGE,
};
...
struct econet_queue_entry {
	union {
		...
		struct {
			...
			struct sk_buff *skb;
			enum econet_dma_map_type dma_type;
		};
	};
	dma_addr_t dma_addr;
	u16 dma_len;
...
static inline u16 econet_qdma_get_txq(struct econet_qdma *qdma, u16 qid)
{
